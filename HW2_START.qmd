...
**Instructions:** Run 1:1 nearest-neighbor propensity score matching (logistic method). 
```{r}
...
Using the matched sample (Mahalanobis method), estimate the effect of treatment on the change in incumbent vote share (`change_liberal`).
...
**3C:** Estimate a fixed effects model using `lm()` with fixed effects added for `precinct` and `year`
